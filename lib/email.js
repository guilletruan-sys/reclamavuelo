  ...
  <div style="max-width:600px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
    ...
    <div style="background:#f8fafc;padding:20px 32px;border-top:1px solid #e2e8f0;text-align:center;">
      <p style="color:#94a3b8;font-size:12px;margin:0 0 6px;">
        ReclamaVuelo · Reglamento CE 261/2004 · Sin costes iniciales
      </p>
...
export async function sendConfirmacionUsuario({ to, nombre, ref, vuelo, ruta, compensacion, decision, uploadUrl }) {
  ...
  const content = `
    ...
    <p style="color:#64748b;font-size:15px;line-height:1.7;margin:0 0 24px;">
      Hola <strong>${nombre}</strong>, hemos analizado los datos de tu vuelo y ${esRevisar
        ...
        : 'nuestro sistema ha determinado que tienes derecho a compensación bajo el Reglamento CE 261/2004.'}
    </p>
...
export async function sendNuevaReclamacionInterna({ ref, decision, confianza, compensacion, vuelo, ruta, fecha, incidentType, nombre, email, telefono, pasajeros, razonamiento, factores, uploadUrl }) {
  ...
  const content = `
    ...
    ${ctaButton('Ver enlace de documentación del cliente →', uploadUrl)}
  `;
...
export async function sendExpedienteKmaleon({ ref, nombre, docNumber, email, telefono, vuelo, fecha, ruta, incidentType, compensacion, pasajeros, validaciones, razonamiento }) {
  const camposKmaleon = [
    ...
    dataRow('Tipo de expediente', 'Reclamación aérea CE 261/2004'),
