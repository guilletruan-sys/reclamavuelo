...
const DOC_TYPES = {
  ...
  pir:      { nombre: 'PIR (parte de irregularidad)', icon: '🧳' },
  medical:  { nombre: 'Parte médico',              icon: '🏥' },
};

// Alias ids usados por el frontend nuevo -> keys internos
const FRONTEND_ALIAS = {
  boarding: 'embarque',
  booking:  'reserva',
  receipts: 'gastos',
};
...
function normalizeDocType(t) {
  return FRONTEND_ALIAS[t] || t;
}

// Parsea un data URL "data:<mime>;base64,<data>" -> { mimeType, base64 }
 ...
 */
/**
 * Valida un solo documento a partir de un dataUrl (usado por la UI nueva
 * de aportar-documentos con modo validateOnly).
 * @param {string} docType - id del doc (dni, boarding, booking, receipts, pir, medical…)
 * @param {string} dataUrl - "data:<mime>;base64,<data>"
 * @param {Object} caseData
 * @returns {Promise<{ok:boolean, error?:string, extracted?:object}>}
 */
export async function validateSingle(docType, dataUrl, caseData = {}) {
  const parsed = parseDataUrl(dataUrl);
  if (!parsed) {
    return { ok: false, error: 'dataUrl inválido' };
  }
  const tipo = normalizeDocType(docType);

  // En modo demo (sin API key) aceptamos el doc automáticamente.
  ...
  try {
    const result = await validateSingleDocument(
      { tipo, mimeType: parsed.mimeType, base64: parsed.base64 },
      caseData || {},
    );
    if (result.ok) return { ok: true, extracted: result.datosExtraidos || {} };
    return { ok: false, error: result.mensaje || 'Documento rechazado' };
  } catch (err) {
    return { ok: false, error: err.message || 'Error al validar el documento' };
  }
}
...
Datos del caso: Titular esperado: ${caseData.nombre}`,
    ...
    pir: `Verifica este PIR (Property Irregularity Report / parte de irregularidad de equipaje):
- ¿Es un PIR emitido por una aerolínea o un servicio de handling del aeropuerto?
- ¿Tiene membrete/logo de aerolínea o aeropuerto?
- ¿Incluye número de referencia del PIR, fecha del incidente, datos del pasajero y descripción del equipaje (perdido/dañado/retrasado)?
- ¿Es legible?
- Extrae: número de PIR, fecha, estado del equipaje (lost/damaged/delayed), descripción, pasajero, aerolínea.
Datos del caso: Titular esperado ${caseData.nombre || 'N/D'}, Vuelo ${caseData.vuelo || 'N/D'}, Fecha ${caseData.fecha || 'N/D'}`,

    medical: `Verifica este parte o informe médico:
- ¿Es un documento médico emitido por un profesional sanitario (hospital, clínica, ambulatorio)?
- ¿Tiene membrete, sello o identificación del centro médico?
- ¿Incluye nombre del médico y/o número de colegiado?
- ¿Incluye fecha, diagnóstico o descripción de la lesión, y datos del paciente?
- ¿Es legible?
- Extrae: nombre del médico, número de colegiado, centro médico, fecha, diagnóstico, paciente.
Datos del caso: Paciente esperado ${caseData.nombre || 'N/D'}, Fecha del vuelo ${caseData.fecha || 'N/D'}`,
