...
export default function ResultCard({ context, disabled, answer, onAnswer }) {
  const [showReasoning, setShowReasoning] = useState(false);
  const result = context?.result;
  const verifyMeta = context?.verifyMeta;
  ...
  const isNoClaim = status === 'NO_RECLAMABLE';
  ...
  const regulation =
    result.regulation || (tipo === 'equipaje' || tipo === 'lesiones' ? 'Convenio de Montreal' : 'CE 261/2004');
  ...
  return (
    <div className="fade-up" style={{ marginTop: tokens.s3 }}>
      {/* Card principal */}
      <div style={{
      ...
      {summary && (
        <div style={{
          ...
          <p style={{ fontSize: 15, lineHeight: 1.6, color: tokens.navy700, margin: 0 }}>{summary}</p>

          {factors.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: tokens.s3 }}>
              {factors.map((f, i) => (
      ...
      {nextStep && (
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 14, color: tokens.navy700, lineHeight: 1.5 }}>{nextStep}</div>
          </div>
        </div>
      )}
        ...
        <div style={{ marginTop: tokens.s3, fontSize: 13, lineHeight: 1.6, color: tokens.navy900 }}>
          {verifyMeta && (() => {
            ...
            if (demo) {
              ...
              sourceColor = '#f59e0b';
              sourceExplanation = 'AviationStack NO devolvió datos del vuelo. Limitación conocida del plan gratuito: solo verifica vuelos actualmente activos o próximos a salir, no vuelos pasados. Claude confió en lo que declaró el pasajero.';
            }
      ...
      {/* Continuar (si reclamable / revisar). NO_RECLAMABLE: el step `next` ya va a endNoClaim */}
      {!isNoClaim && !disabled && (
        <div style={{ marginTop: tokens.s5 }}>
          <Button
            variant="primary"
            onClick={() => onAnswer(true, '✓ Continúo con la reclamación')}
            style={{ width: '100%' }}
          >
